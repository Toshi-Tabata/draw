...
function toggleFinger() {
    fingerDrawOff = !fingerDrawOff;
}

function forceInput(input) {
    let forceIn = document.getElementById("force");
    forceIn.innerText = input;
}
...
function setPenThickness(event) {
    ...
    context.lineWidth = width;
    console.log("changed input width to: " + inputWidth);
...
function getCoords(event) {
    ...
    // TODO: nested more deeply than I'd like
    if (event.touches) {
        let touchEvent = event.touches[0];
        if (touchEvent && typeof touchEvent["force"] !== "undefined") {
            if (touchEvent["force"] > 0) {
                let force = touchEvent["force"];
                width = (Math.abs(Math.log(force + 1)) * 3 + parseFloat(inputWidth));
                forceInput(`Force applied was: ${width}`)
            }
...
function setResolution(width, height) {
    ...
    const scale = window.devicePixelRatio;
    console.log("current device's scale was: " + scale);
